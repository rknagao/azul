...
def compare_nps_categorical(df, nps_column, cat_column):
    ...
    df_q1 = pd.pivot_table(data=df_q1, values='Id', index=cat_column, columns=nps_column, aggfunc='count', margins=True, margins_name='total_q1', fill_value=0).iloc[:-1].reset_index()
    df_q1['nps_q1'] = (df_q1['promotor'] - df_q1['detrator'])/df_q1['total_q1']

    df_q2 = pd.pivot_table(data=df_q2, values='Id', index=cat_column, columns=nps_column, aggfunc='count', margins=True, margins_name='total_q2', fill_value=0).iloc[:-1].reset_index()
